...
public:
  ...
  uint64_t getMask(uint8_t pos);

  const uint64_t getBishopAttacks1(uint8_t square, uint64_t occupancy);

  const uint64_t getRookAttacks1(uint8_t square, uint64_t occupancy);
